...
(defn can-make-ransom-note
  ...
  (let [message-count (utils/count-non-whitespace-chars (utils/read-message message-file-name))
    ...
    (cond
      (empty? message-count) true                           ;;Message doesn't contain any non whitespace characters
      (zero? magazine-size) false                           ;;Magazine fail is empty, so we can't make any note
      :else
